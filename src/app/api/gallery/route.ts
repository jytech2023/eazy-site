...
export async function GET(request: Request) {
  ...
  // Get total stats (fire both queries in parallel)
  const [publishedSites, [statsRow]] = await Promise.all([
    // Get published sites with views, newest first
    db
      .select({
        id: sites.id,
        title: sites.title,
    ...
    db
      .select({
        totalSites: count(sites.id),
        totalViews: sum(sites.views),
      })
      .from(sites)
      .where(eq(sites.published, true)),
  ]);
  ...
  return NextResponse.json({
    ...
    stats: {
      totalSites: statsRow?.totalSites || 0,
